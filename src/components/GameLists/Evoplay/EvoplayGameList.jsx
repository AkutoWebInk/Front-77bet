...
import styles from './EvoplayGameList.module.css';
import { IoIosArrowForward } from "react-icons/io";
import { NavLink } from 'react-router-dom';
import { getProviderIcon } from '../../../api/providers';
import EvoplayGameCard from './EvoplayGameCard';

export default function EvoplayGameList({ games = [], loading, limit = 12 }) {
  const providerId = 'EVOPLAY';
  const providerName = 'EVOPLAY';
        ...
        {loading ? (
          Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className={styles.cardWrapper}>
              <EvoplayGameCard loadingSkeleton={true} variant="square" />
            </div>
          ))
        ) : (
